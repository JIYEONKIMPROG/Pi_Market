...
const BRAIN_STEM_GLB_PATH = assetPath('models/glTF-Sample-Models/2.0/BrainStem/glTF-Binary/BrainStem.glb');
...
const findPrimitivesWithVariant = (model, variantName) => {
    const result = new Array();
    for (const primitive of model[$primitivesList]) {
        if (primitive.variantInfo != null && model.hasVariant(variantName) &&
            primitive.variantInfo.has(model[$variantData].get(variantName).index)) {
    ...
    suite('Static Primitive With Variant', () => {
        ...
        test('Switching to current variant', async () => {
            ...
            for (const primitive of primitives) {
                materials.push(await primitive.enableVariant('Yellow Yellow'));
            }
            expect(materials).to.not.be.empty;
            expect(materials.find((material) => {
        ...
        test('Switching to variant and then switch back', async () => {
            ...
            materials = new Array();
            for (const primitive of primitives) {
                materials.push(await primitive.enableVariant('Purple Yellow'));
            }
            expect(materials.find((material) => {
...
//# sourceMappingURL=primitive-node-spec.js.map