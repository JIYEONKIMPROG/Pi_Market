...
describe('NFTStorageMetaplexor', () => {
    ...
    describe('withSigner', () => {
        it('creates an instance with an AuthContext backed by the given signMessage function', async () => {
            const signMessage = async (msg) => nacl.sign.detached(msg, secretKey);
    ...
    describe('storeNFTFromFilesystem', () => {
        it('loads an NFT from disk and posts two CARs to /metaplex/upload', async () => {
            ...
            const metadataPath = path.join(__dirname, 'fixtures', 'nfts', '01-simple-example', 'token.json');
            ...
            expect(result.metadata['image']).to.eq(`https://dweb.link/ipfs/${result.assetRootCID}/token.png`);
        });
    });
});
async function signRandomMessage(auth) {
