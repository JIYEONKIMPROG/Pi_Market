 ...
 */
function* makeArweaveBundleUploadGenerator(storage, dirname, assets, env, jwk, walletKeyPair, batchSize) {
    let signer;
    ...
    while (filePairs.length) {
        const result = getBundleRange(filePairs, storage === storage_type_1.StorageType.ArweaveSol ? true : false).then(async function processBundle({ count, size }) {
            ...
            if (storageType === storage_type_1.StorageType.ArweaveSol) {
                ...
                const bytes = dataItems.reduce((c, d) => c + d.getRaw().length, 0);
                const cost = await bundlr.utils.getPrice('solana', bytes);
                loglevel_1.default.info(`${(cost.toNumber() * 2) / exports.LAMPORTS} SOL to upload ${sizeMB(bytes)}MB with buffer. Sending fund txn...`);
