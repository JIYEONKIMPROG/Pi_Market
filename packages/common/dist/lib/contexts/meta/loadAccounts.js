...
const processMints = async (connection, mintList, tempCache, updateTemp, sourceMessage = 'mints') => {
    ...
    for (let i = 0; i < mintList.length; i++) {
        ...
        const edition = await (0, actions_1.getEdition)(mintAddress);
        const newAdd = [await (0, actions_1.getMetadata)(mintAddress), edition];
    ...
    console.log('------> From ', sourceMessage, 'produced', batches.length, 'batches of accounts to pull');
...
exports.processMints = processMints;
const pullMintsMetadata = async (connection, mintList, tempCache, sourceMessage = 'mints') => {
    const updateTemp = (0, exports.makeSetter)(tempCache);
    await (0, exports.processMints)(connection, mintList, tempCache, updateTemp, sourceMessage);
    await postProcessMetadata(tempCache);
    console.log('-------->', sourceMessage, ' metadata processing complete.');
    return tempCache;
};
exports.pullMintsMetadata = pullMintsMetadata;
const pullYourMetadata = async (connection, userTokenAccounts, tempCache) => {
    ...
    return await (0, exports.pullMintsMetadata)(connection, mintList, tempCache, 'User');
};
...
const forEach = (fn, updateTemp) => async (accounts) => {
    for (const account of accounts) {
        await fn(account, updateTemp);
    }
};
const pullAuctionData = async (connection, auction, tempCache) => {
    var _a;
    const updateTemp = (0, exports.makeSetter)(tempCache);
    let cacheKey;
    try {
    ...
    await connection
        ...
        : null);
    await (0, web3_1.getProgramAccounts)(connection, ids_1.AUCTION_ID, {
        filters: [
            {
                memcmp: {
                    offset: 32,
    ...
    }).then(forEach(processAuctions_1.processAuctions, updateTemp));
    return tempCache;
};
exports.pullAuctionData = pullAuctionData;
const pullAuctionSubaccounts = async (connection, auction, tempCache) => {
    ...
    const promises = [
        ...
        }).then(forEach(processVaultData_1.processVaultData, updateTemp)),
        // bid redemptions
        (0, web3_1.getProgramAccounts)(connection, ids_1.METAPLEX_ID, {
            filters: [
                {
        ...
        }).then(forEach(processMetaplexAccounts_1.processMetaplexAccounts, updateTemp)),
        ...
        if (auctionCaches && auctionCaches.keys.length) {
            ...
            for (let i = 0; i < auctionCaches.keys.length; i++) {
                ...
                for (const m of metadata) {
                    const data = m === null || m === void 0 ? void 0 : m.info;
                    if (data === null || data === void 0 ? void 0 : data.collection) {
                        collections.add(data.collection.key);
                    }
                }
            }
            await (0, exports.processMints)(connection, Array.from(collections), tempCache, updateTemp);
